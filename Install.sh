...
sudo rm -rf /Volumes/EFI/refind/{regular-theme,refind-theme-regular}

sleep 1
clear
echo "Downloading Refind Theme to "$HOME" [DONE]"
echo "Deleteting older installed versions (If any) [DONE]"
echo "Copying Theme to /Volumes/EFI/refind/ [NOT DONE]"

sudo cp -r refind-theme-regular /Volumes/EFI/refind/

sleep 1
clear
echo "Downloading Refind Theme to "$HOME" [DONE]"
echo "Deleteting older installed versions (If any) [DONE]"
echo "Copying Theme to /Volumes/EFI/refind/ [DONE]"
echo "Removing unused directories [NOT DONE]"

sudo rm -rf /Volumes/EFI/refind/refind-theme-regular/{src,.git}
...
sudo sed --in-place=".bak" 's/^\s*include/# (disabled) include/' /Volumes/EFI/refind/refind.conf
...
" | sudo tee -a /Volumes/EFI/refind/refind.conf &> /dev/null
